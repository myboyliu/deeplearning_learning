...
my_file_4= project_dir+"/data/invoice/img/"+file_name+"_new_4.jpg"
...
img = newimg
#
image, cnts, hierarchy = cv2.findContours(img.copy(), cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)
...
#方向识别
#print(c)
#img1 = cv2.drawContours(img, c, -1, (255, 0, 0),cv2.FILLED)

img1 = cv2.fillConvexPoly(img,c,(0,0,0))
img1= cv2.cvtColor(img1,cv2.COLOR_BGR2GRAY)
img1, cnts2, hierarchy = cv2.findContours(img1.copy(), cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)
...
for c in cnts2:
    ...
    areamap2[area]=c
areamap2_sort_keys = sorted(areamap2.keys())

for i in range(-3,-2) :
    c = areamap2[areamap2_sort_keys[i]]
    img1 = cv2.drawContours(img1, c, -1, (255, 0, 0), 2)

cv2.imwrite(my_file_4, img1)
